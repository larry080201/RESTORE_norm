...
def run(CSV, mfname, pos_idx, save_dir, floor):
 ...
 #   source_df = source_df.filter(regex="^Intensity_IntegratedIntensity_.*$")
 #   source_df.columns = source_df.columns.str.replace("Intensity_IntegratedIntensity_", "")
    ## need to add a arbitary colomn called "scene"
 #   source_df.insert(0, 'scene', 'scene1')
    ...
    path = f'{save_dir}/{source}_logfile.txt'
    sys.stdout = open(path, 'w')
